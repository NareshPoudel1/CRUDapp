...
namespace CRUDapp.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public async Task<IActionResult> Delete(Product viewModel)
        {
            ...
            if (product is not null)
            {
                dbContext.Products.Remove(product); // Corrected to remove 'product' instead of 'viewModel'
                await dbContext.SaveChangesAsync();
            }

            return RedirectToAction("List", "Products"); // Corrected the return statement
        }
    }
}
